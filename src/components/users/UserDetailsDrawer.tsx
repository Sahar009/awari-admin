    ...
    <div className="fixed inset-0 z-50 flex">
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    {kycDocuments.map((doc) => (
                      ...
                      >
                        ...
                        {/* Document actions - Note: documentUrl needs to be added to API response */}
                        <div className="flex gap-2">
                          <button
                            onClick={() => {
                              // TODO: Replace with actual document URL from API
                              const docUrl = (doc as any).documentUrl || '#';
                              setViewingDocument({ url: docUrl, type: doc.documentType, title: `${doc.documentType} - ${user?.firstName} ${user?.lastName}` });
                            }}
                            className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg border border-indigo-500/20 bg-indigo-500/10 px-3 py-1.5 text-xs font-medium text-indigo-600 transition hover:bg-indigo-500/20 dark:text-indigo-400"
                            title="View document"
                          >
                          ...
                          <button
                            ...
                            className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
      ...
      {viewingDocument && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm transition-all duration-300">
          <div className="relative flex h-full max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-slate-900">
            {/* Modal Header */}
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4 dark:border-slate-800">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="inline-flex h-9 items-center gap-2 rounded-lg border border-slate-200 px-4 text-xs font-semibold text-slate-600 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
                >
                ...
                <button
                  ...
                  className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-slate-500 transition hover:bg-slate-50 hover:text-slate-900 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white"
                >
                  <X className="h-5 w-5" />
                </button>
              </div>
